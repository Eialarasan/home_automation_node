...
app.post('/addHome', Validation, (req, res) => {
  // Your route logic here
  res.status(200).json({ response_code: 0, response_message: "Home added successfully" });
});

// Jest test case
describe('Validation Middleware', () => {
  it('should pass validation when homeName is present', () => {
    const req = {
      body: {
        homeName: 'My Home',
      },
  ...
  it('should send an error response when homeName is missing', () => {
    const req = {
      body: {},
      user: {
        userId: '6789',
    ...
    expect(res.send).toHaveBeenCalledWith({
      response_code: 2,
      response_message: "Home name is missing",
      response_code: 1,
    });
  });
});
